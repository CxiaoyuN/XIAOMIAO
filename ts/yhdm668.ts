...
export default class yhdm668 implements Handle {
  ...
  async getHome() {
    ...
    try {
      ...
      return $('.module-items .module-item').toArray().map(item => {
        ...
        return <IMovie>{
          id: a.attr('href') ?? '',
          title: a.attr('title') || a.find('.module-poster-item-title').text().trim() || a.text().trim(),
  ...
  async getDetail() {
    ...
    try {
      ...
      const tabs = $('.module-tab-items-box .module-tab-item, .module-tab .module-tab-item').toArray().map(item =>
        $(item).text().trim()
      );
  ...
  async parseIframe() {
    ...
    try {
      ...
      const m3u8Match = scriptContent.match(/(?:url|videoUrl)\s*[:=]\s*["'](.*?\\.m3u8.*?)["']/i);
  ...
  private base64Decode(input: string) {
    ...
    let output = "";
    if (str.length % 4 === 1) return output;
    let bs = 0;
    for (
