export default class LiuYueTingShu implements Handle {
  ...
  async getHome() {
    ...
    for (const cat of categories.slice(0, 6)) { // 首页只展示前6个分类
      const html = await req(`${env.baseUrl}${cat.id}`)
      const $ = kitty.load(html)
      const items = $('.album-item').toArray().map(item => {
        ...
        return { id, title, cover, desc, remark }
      })
  ...
  async getCategoryDetail() {
    ...
    const $ = kitty.load(html)
    return $('.album-item').toArray().map(item => {
      const title = $(item).find('.book-item-name a').text().trim()
      const id = $(item).find('.book-item-name a').attr('href') ?? ''
      const cover = $(item).find('.book-item-img img').attr('src') ?? ''
      const remark = $(item).find('.book-item-status').text().trim()
  ...
  async getDetail() {
    ...
    const playlist = [{
      title: '播放列表',
      videos: $('.play-list.list li a').toArray().map(a => {
